    ...
    <div class="page-wrap">
        ...
        <div class="form-card">
            ...
            <form method="POST">
                ...
                <div class="form-row">
                    <div class="form-group"><label>Member</label><select name="member_id" required>
                            <option value="">— select —</option><?php foreach ($memberList as $mb): ?>
                                <option value="<?= $mb['id'] ?>">
                                    <?= htmlspecialchars($mb['last_name'] . ', ' . $mb['first_name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select></div>
                    <div class="form-group"><label>Event</label><select name="event_id" required>
                            <option value="">— select —</option><?php foreach ($eventList as $ev): ?>
                                <option value="<?= $ev['id'] ?>"><?= htmlspecialchars($ev['event_name']) ?>
                                    (<?= $ev['event_date'] ? date('M j', strtotime($ev['event_date'])) : '—' ?>)</option>
                            <?php endforeach; ?>
                        </select></div>
                    ...
                    <div class="form-group"><label>Time Note</label><input type="text" name="time_note"
                            placeholder="e.g. 7:30"></div>
        ...
        <form method="GET" class="search-bar">
            ...
            <?php if ($search): ?><a href="manage_attendance.php"
                    style="color:#888;font-size:0.8rem;">Clear</a><?php endif; ?>
        </form>
                ...
                <tbody>
                    <?php foreach ($records as $r): ?>
                        <tr id="view-<?= $r['id'] ?>">
                            ...
                            <td><span
                                    class="status-pill <?= strtolower($r['status']) ?>"><?= htmlspecialchars($r['status']) ?></span>
                            </td>
                            <td><?= $r['time_note'] ? htmlspecialchars($r['time_note']) : '—' ?></td>
                            <td class="actions">
                                <button class="btn-edit" onclick="toggleEdit(<?= $r['id'] ?>)">Edit</button>
                                <form method="POST" style="display:inline" onsubmit="return confirm('Delete this record?')">
                                    <input type="hidden" name="action" value="delete"><input type="hidden" name="att_id"
                                        value="<?= $r['id'] ?>">
                        ...
                        <tr id="edit-<?= $r['id'] ?>" style="display:none;background:#141416">
                            <form method="POST"><input type="hidden" name="action" value="update"><input type="hidden"
                                    name="att_id" value="<?= $r['id'] ?>">
                                <td><?= $r['id'] ?></td>
                                <td>
                                    <select name="member_id" required>
                                        <?php foreach ($memberList as $mb): ?>
                                            <option value="<?= $mb['id'] ?>" <?= $mb['id'] == $r['member_id'] ? 'selected' : '' ?>>
                                                <?= htmlspecialchars($mb['last_name'] . ', ' . $mb['first_name']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <td>
                                    <select name="event_id" required>
                                        <?php foreach ($eventList as $ev): ?>
                                            <option value="<?= $ev['id'] ?>" <?= $ev['id'] == $r['event_id'] ? 'selected' : '' ?>>
                                ...
                                <td>
                                    <select name="status" required>
                                        <option value="Attending" <?= $r['status'] === 'Attending' ? 'selected' : '' ?>>Attending
                                        </option>
                                ...
                                <td><input type="text" name="time_note" value="<?= htmlspecialchars($r['time_note'] ?? '') ?>">
                                </td>
                                <td class="actions">
                                    <button type="submit" class="btn-save">Save</button>
                                    <button type="button" class="btn-cancel"
                                        onclick="toggleEdit(<?= $r['id'] ?>)">Cancel</button>
                                </td>
                            </form>
                        </tr>
                    <?php endforeach; ?>
