...
{
    $raw = trim($raw);
    if (stripos($raw, 'Attending') !== false)
        return ['Attending', null];
    if (stripos($raw, 'Away') !== false)
        return ['Away', null];
    if (preg_match('/^Late\s+(\d{1,2}:\d{2})/i', $raw, $m))
        return ['Late', $m[1]];
    if (preg_match('/^Leave\s+(\d{1,2}:\d{2})/i', $raw, $m))
        return ['Leave', $m[1]];
